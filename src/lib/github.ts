...
export async function getGitHubClient(): Promise<Octokit> {
  ...
  try {
    ...
    throw new Error(
      "An unknown error occurred while authenticating with GitHub",
    );
  }
}
...
export async function getRepoActivity(
  ...
  try {
    ...
    const commits = await Promise.all(
      commitsResponse.data.slice(0, 15).map(async (commit) => {
    ...
    const pullRequests = await Promise.all(
      prsResponse.data
        .filter((pr) => pr.user?.login?.toLowerCase() === username)
        .slice(0, 5)
        .map(async (pr) => {
